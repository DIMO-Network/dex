...
func (t *templates) web3login(r *http.Request, w http.ResponseWriter, challengeURL string, verifyURL string, authID string, infuraID string, rpcUrl string) error {
	data := struct {
		ChallengeURL string
		VerifyURL    string
		AuthID       string
		...
		InfuraID     string
		RpcURL       string
	}{challengeURL, verifyURL, authID, r.URL.Path, infuraID, rpcUrl}
